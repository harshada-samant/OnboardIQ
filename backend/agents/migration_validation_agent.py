...
def _get_source_row_counts() -> dict:
    ...
    try:
        ...
        # If registry is list, iterate
        records = registry if isinstance(registry, list) else registry.get("registry", [])
        for rec in records:
            logical = rec.get("logical_table")
            total_rows = 0
            for sf in rec.get("source_files", []):
                fname = sf.get("file_name")
                file_path = config.INPUT_DIR / fname
                if file_path.exists():
                    if file_path.suffix.lower() == ".csv":
                        ...
                        # Simple count estimate or regex lines
...
def run_migration_validation_agent(context: dict, verbose: bool = True) -> dict:
    ...
    source_counts = _get_source_row_counts()
