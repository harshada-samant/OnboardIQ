...
def _load_logical_table_df(logical_table_record: dict, source_files: list, storage=None) -> pd.DataFrame:
    """
    ...
    for sf in logical_table_record.get("source_files", []):
        ...
        try:
            if storage:
                ...
                if file_buf is None:
                    print(f"  ! [profiling] Failed to read {file_name} from storage")
                    continue
...
def run_profiling_agent(context: dict, verbose: bool = True) -> dict:
    ...
    for reg_entry in file_registry:
        ...
        df = _load_logical_table_df(reg_entry, source_files, storage=storage)
        if df.empty:
            if verbose:
        ...
        if relationships:
            for rel in relationships:
                ...
                to_df = _load_logical_table_df(target_reg, source_files, storage=storage)
                if to_df.empty:
                    continue
