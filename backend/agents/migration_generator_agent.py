...
def _build_file_registry_index(context: dict) -> dict:
    ...
    for entry in (context.get("file_registry") or []):
        ...
        if config.USE_S3_SOURCE:
            bucket = config.S3_BUCKET or "onboardiq-bucket"
            prefix = f"workspaces/users/{username}/uploads"
            paths = [
                f"s3://{bucket}/{prefix}/{sf['file_name']}"
...
S3 ACCESS RULE:
    If any of the source_files paths start with "s3://", the script MUST load the DuckDB httpfs extension and configure AWS S3 connection settings using environment variables (AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY or their S3_ prefixed overrides S3_AWS_ACCESS_KEY_ID, S3_AWS_SECRET_ACCESS_KEY) BEFORE reading from the files.
    Example block to inject after connection initialization:
    ```python
    if any(str(f).startswith("s3://") for f in source_files):
        con.execute("INSTALL httpfs;")
        con.execute("LOAD httpfs;")
        s3_region = os.environ.get("AWS_REGION") or os.environ.get("AWS_DEFAULT_REGION", "us-east-1")
        con.execute(f"SET s3_region='{s3_region}';")
        s3_access = os.environ.get("S3_AWS_ACCESS_KEY_ID") or os.environ.get("AWS_ACCESS_KEY_ID")
        s3_secret = os.environ.get("S3_AWS_SECRET_ACCESS_KEY") or os.environ.get("AWS_SECRET_ACCESS_KEY")
