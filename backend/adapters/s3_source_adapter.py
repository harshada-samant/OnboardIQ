...
class S3SourceAdapter:
    def __init__(self, bucket: str, prefix: str):
        ...
        # Fallback to default AWS env keys if S3-specific keys are absent
        if "aws_access_key_id" not in s3_kwargs and os.getenv("AWS_ACCESS_KEY_ID"):
            s3_kwargs["aws_access_key_id"] = os.getenv("AWS_ACCESS_KEY_ID")
        if "aws_secret_access_key" not in s3_kwargs and os.getenv("AWS_SECRET_ACCESS_KEY"):
