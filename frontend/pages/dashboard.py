...
def dashboard_page():
    ...
    # ── Main content ─────────────────────────────────────────────────────────
    with ui.row().classes('w-full no-wrap q-pa-xl gap-6'):
        # Left panel placeholder (hidden until needed)
        ui.column().classes('hidden')

        # Center content card
        with ui.column().classes('col-grow'):
            with ui.card().classes('q-pa-xl bg-white w-full').style(
                'border: 1px solid #e2e8f0; border-radius: 16px; box-shadow: 0 4px 20px rgba(0,0,0,0.04);'
            ):
                ui.label(f'Welcome back, {username}!') \
                  .classes('text-h4 text-weight-bold q-mb-xs') \
                  .style('color: #0f172a;')
                ui.label('Your secure isolated workspace is ready.') \
                  .classes('text-subtitle1 q-mb-lg') \
                  .style('color: #64748b;')
        ...
        # Right panel placeholder (hidden until needed)
        ui.column().classes('hidden')
