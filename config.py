...
def load_and_validate_env(verbose: bool = False) -> None:
    ...
    if provider_override in {"groq", "bedrock"}:
        LLM_PROVIDER = provider_override
        if LLM_PROVIDER == "groq":
            if not has_groq:
                raise EnvironmentError(
                    "LLM_PROVIDER=groq is set, but GROQ_API_KEY is missing from environment/.env."
                )
            print("[config] LLM_PROVIDER=groq override detected. Using Groq directly.")
            return

        print("[config] LLM_PROVIDER=bedrock override detected. Using Bedrock unless connectivity fails.")
...
# ── S3 source toggle (merged from Project A) ─────────────────────────────────
...
def s3_input_prefix(username: str, job_id: str = None) -> str:
    """Return the S3 prefix for a user's input files, mirroring local workspaces."""
