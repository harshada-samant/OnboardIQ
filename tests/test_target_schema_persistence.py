...
def run_tests():
    ...
    setup_temp_schema("target_schema.json")
    ...
    try:
        ...
        try:
            update_user_target_schema(non_existent_id, "target_schema.json")
        ...
        # Save target_schema.json for user1
        print("Updating user1 schema to 'target_schema.json'...")
        update_user_target_schema(user1_id, "target_schema.json")
        
        # Verify isolation: user2 must still be None
        assert get_user_target_schema(user1_id) == "target_schema.json", "user1 schema did not save correctly"
        ...
        # Verify isolation: user1 must remain "target_schema.json"
        assert get_user_target_schema(user1_id) == "target_schema.json", "user1 schema was unexpectedly modified"
        ...
        assert schema_user1_persisted == "target_schema.json", "user1 schema failed persistence check"
        ...
        with get_db_connection() as conn:
            ...
            assert raw_val1 == "target_schema.json", f"Expected exact value 'target_schema.json', found: {raw_val1}"
